...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    int fd = open("/home/router/Desktop/fuse-tutorial-2018-02-04/example/ubuntu-sample.log", O_APPEND|O_CREAT|O_RDWR, 0777);
    char line[100]={0};
    sprintf(line, "%016x    %08x    %08x    %08x    %d\n",offset, size, 0, size, tnow.tv_sec);

    int n = write(fd, line, strlen(line));
    if (n<0)
        log_msg("errno: %s", strerror(errno));
    close(fd);
	     ...
	     struct fuse_file_info *fi)
{
    ...
    int fd = open("/home/router/Desktop/fuse-tutorial-2018-02-04/example/ubuntu-sample.log", O_APPEND|O_CREAT|O_RDWR, 0777);
    char line[100]={0};
    sprintf(line, "%016x    %08x    %08x    %08x    %d\n",offset, size, 1, size, tnow.tv_sec);
